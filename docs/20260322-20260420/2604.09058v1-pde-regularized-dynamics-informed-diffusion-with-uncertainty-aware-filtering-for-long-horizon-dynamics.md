...
tags: ["query:tsf"]
score: 9.0
evidence: 动力学启发扩散模型用于长周期时空预测
tldr: 针对长程时空预测中累积误差和物理一致性缺失的问题，本文提出PDYffusion框架。该方法结合了PDE正则化插值器以确保物理约束，并利用无迹卡尔曼滤波（UKF）预测器来建模不确定性并抑制误差累积。实验证明该方法在保持物理一致性的同时，显著提升了长程预测的准确性与鲁棒性。
source: arxiv
...
motivation: 现有的长程时空预测模型难以处理预测过程中的累积误差、噪声放大以及物理规律一致性缺失的问题。
method: 提出PDYffusion框架，包含一个引入微分算子的PDE正则化插值器和一个基于UKF的不确定性感知预测器。
result: 在多个动力学数据集上，该方法在CRPS、MSE和SSR指标上均优于现有模型，表现出更强的预测稳定性和准确度。
conclusion: PDYffusion通过整合物理约束与不确定性过滤，为长程动力学预测提供了一个平衡准确性与鲁棒性的有效方案。
---

## 摘要
由于累积误差、噪声放大以及现有模型缺乏物理一致性，长程时空预测仍然是一个具有挑战性的问题。虽然扩散模型为不确定性建模提供了一个概率框架，但传统方法通常依赖于均方误差目标，且无法捕捉受物理定律支配的潜在动力学。在这项工作中，我们提出了 PDYffusion，这是一个动力学感知的扩散框架，它集成了基于偏微分方程（PDE）的正则化和不确定性感知预测，以实现稳定的长期预测。该方法由两个关键组件组成：一个 PDE 正则化插值器和一个基于无迹卡尔曼滤波（UKF）的预测器。插值器引入了微分算子以强制执行物理一致的中间状态，而预测器则利用无迹卡尔曼滤波来显式建模不确定性，并减轻迭代预测过程中的误差累积。我们提供了理论分析，证明所提出的插值器满足 PDE 约束的光滑性质，且预测器在所提出的损失函数下收敛。在多个动力学数据集上的广泛实验表明，PDYffusion 在 CRPS 和 MSE 方面取得了优异的性能，同时保持了由 SSR 衡量的稳定不确定性行为。我们进一步分析了预测准确性与不确定性之间固有的权衡，表明我们的方法为长程预测提供了一个平衡且鲁棒的解决方案。
