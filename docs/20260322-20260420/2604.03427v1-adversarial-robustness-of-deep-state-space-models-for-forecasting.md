...
title_zh: 深度状态空间模型在预测任务中的对抗鲁棒性
authors: "Sribalaji C. Anand, George J. Pappas"
date: 2026-04-03
pdf: "https://arxiv.org/pdf/2604.03427v1"
tags: ["query:tsf"]
score: 9.0
evidence: 深度状态空间模型在时间序列预测中的对抗鲁棒性
tldr: 本文深入探讨了深度状态空间模型（SSM）在时间序列预测中的对抗鲁棒性，重点分析了Spacetime架构。研究首先证明了该架构在处理自回归过程时具备表示最优卡尔曼预测器的独特能力，随后将鲁棒预测器设计建模为针对隐蔽攻击者的Stackelberg博弈，并通过对抗训练求解。通过推导预测误差的理论界限，揭示了系统不稳定性及维度对模型脆弱性的影响。实验表明，利用局部线性行为的无梯度攻击比传统PGD攻击更具威胁，为构建鲁棒预测模型提供了理论依据和设计准则。
source: arxiv
...
motivation: 尽管状态空间模型在时间序列预测中表现优异，但其在对抗扰动下的鲁棒性及其背后的理论机制尚未得到充分研究。
method: 从控制理论视角出发，将鲁棒预测器设计建模为受限对抗博弈，并推导出关于系统稳定性和维度的对抗误差闭式界限。
result: "理论分析确定了放大模型脆弱性的关键因素，实验显示无需梯度的模型无关攻击在Monash数据集上造成的误差比PGD攻击高出至少33%。"
conclusion: 研究证明了Spacetime SSM的表示优势，并提出了通过优化系统动力学特性来增强深度预测模型鲁棒性的指导原则。
---

## 摘要
用于时间序列预测的状态空间模型（SSM）在基准数据集上表现出强大的经验性能，但其在对抗扰动下的鲁棒性仍缺乏深入理解。我们通过控制理论的视角来填补这一空白，重点关注近期提出的 Spacetime SSM 预测器。我们首先证明，当底层数据生成过程是自回归的时，仅解码器（decoder-only）的 Spacetime 架构可以表示最优卡尔曼预测器（Kalman predictor）——这是其他 SSM 都不具备的特性。在此基础上，我们将鲁棒预测器设计建模为针对受检测预算限制的最坏情况隐蔽对抗者的 Stackelberg 博弈，并通过对抗训练进行求解。我们推导了对抗预测误差的闭式界限，揭示了开环不稳定性、闭环不稳定性以及解码器状态维度如何分别放大脆弱性，从而为鲁棒预测器设计提供了可操作的原则。最后，我们展示了即使无法访问预测器的对抗者，也可以利用模型的局部线性输入输出行为构建有效的攻击，从而完全绕过梯度计算。在 Monash 基准数据集上的实验表明，无需任何梯度计算的无模型攻击所造成的误差，比小步长的投影梯度下降（PGD）至少高出 33%。
