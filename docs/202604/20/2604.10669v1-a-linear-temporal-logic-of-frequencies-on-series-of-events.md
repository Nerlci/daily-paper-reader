...
motivation: 现有的逻辑框架难以自然且严谨地表达和处理事件序列中的频率属性及定量分布问题。
method: 引入了基于标准 Kripke 语义的原始模态量词和度量敏感算子，构建了全新的 LTLF 逻辑框架。
result: 实现了在单一逻辑结构中对实际观测频率与理想分布之间关系的显式形式化表达与分析。
